...
export function TradesTable({ trades, prices, onRequestCloseTrade, onRequestEditTrade, onRequestDeleteTrade }: TradesTableProps) {
  return (
      ...
      <Table>
        ...
        <TableBody>
          {trades.map((trade) => {
            ...
            const unrealizedGainLoss = trade.status === 'open' && currentPrice !== undefined
              ? (trade.premium - currentPrice) * (trade.contracts ?? 1)
              : null;
              ...
              <TableRow key={trade.id}>
                ...
                <TableCell>
                  {unrealizedGainLoss !== null ? (
                    <span className={unrealizedGainLoss >= 0 ? 'text-green-600' : 'text-red-600'}>
