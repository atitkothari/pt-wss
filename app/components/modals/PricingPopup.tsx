...
export const PricingPopup = ({
    ...
    return (
      <>
        <span className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900">
          <span className="line-through text-gray-300 font-normal">
            {pricing.originalPriceStr}
          </span>
          <span className="line-through text-gray-600 font-normal mx-1 sm:mx-2">
            {pricing.limitedTimePriceStr}
          </span>
    ...
    if (cycle === "yearly") {
      ...
      return (
        <p className="text-green-600 text-xs md:text-sm mt-2">
          <span className="line-through text-gray-300 font-normal">
            {originalYearlyTotal.toFixed(0)}
          </span>
          <span className="line-through text-gray-600 font-normal mx-1">
            {limitedTimeYearlyTotal.toFixed(0)}
          </span>
  ...
  return (
    <>
      ...
      >
        <div
          className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto animate-in fade-in-0 zoom-in-95 duration-300"
          ...
          <div className="flex justify-between items-start p-4 sm:p-6 border-b border-gray-200">
            <div className="flex-1 min-w-0">
              ...
              <p className="text-gray-600 mt-2 text-sm sm:text-base leading-relaxed">
                Already discounted prices + extra 25% off with coupon code{" "}
                <span className="font-mono bg-yellow-100 text-yellow-800 px-1 sm:px-2 py-0.5 sm:py-1 rounded text-xs sm:text-sm font-semibold break-words">
                  LABORDAY25
                </span>
              </p>
            </div>
              ...
              <CardHeader className="text-center pb-4 sm:pb-6 border-b px-4 sm:px-6">
                ...
                <div className="mt-3 p-2 sm:p-3 bg-green-50 border border-green-200 rounded-lg">
                  <p className="text-green-700 text-xs sm:text-sm font-medium leading-relaxed">
                    💰 Save {discountedPricing[billingCycle].savings} per month
                    with coupon code LABORDAY25
                  </p>
                </div>
                  ...
                  {status === "active" || status === "paused" ? (
                    <Button
                      onClick={() => {
                        ...
                        sendAnalyticsEvent({
                          event_name:
                            "pricing_popup_manage_subscription_clicked",
                          event_category: "Pricing Popup",
                          event_label: triggerSource,
                          custom_parameters: {
                            billing_cycle: billingCycle,
