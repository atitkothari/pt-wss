...
export default function CashSecuredPutScreenerPage() {
  ...
  useEffect(() => {
    ...
    if (params.has('type')) {
      ...
      router.replace(`/cash-secured-put-screener?${params.toString()}`);
    }
  }, [searchParams, router]);
        ...
        <div className="max-w-screen-2xl mx-auto p-4">
          <div className="mb-4">
            <div className="flex justify-between items-center mb-6">
              <h1 className="text-2xl font-bold">Cash Secured Put Screener</h1>
              <Link 
                href="/covered-call-screener" 
                className="text-blue-600 hover:text-blue-800 text-sm font-medium"
              >
                Switch to Covered Call Screener →
              </Link>
            </div>
          ...
          <div className="bg-white p-4 rounded-lg shadow-sm relative">
            <OptionsTableComponent option="put" />
          </div>
          <Footer />
        </div>
      </div>
